...
export const run = async ({ interaction, handler }: SlashCommandProps) => {
	...
	let bots =
		...
		(guild?.members.cache.filter((u) => u.user.bot != true).size ||
			1000000000000000);
	const textComponent = new TextDisplayBuilder().setContent("# Serverinfo");
	const textComponent2 = new TextDisplayBuilder().setContent("eeee");

	const seperatorComponent = new SeparatorBuilder();
	const containerComponent = new ContainerBuilder()
		.addTextDisplayComponents(textComponent)
		.addSeparatorComponents(seperatorComponent)
